"""Evaluate sparse-dense token blocks on SANA-Sprint"""
...
from fae.modules.sparse_dense import apply_sparse_dense_token_blocks, iter_sparse_dense_wrappers
...
def _sparse_dense_runtime_summary(pipe: Any) -> dict[str, Any]:
    wrappers = list(iter_sparse_dense_wrappers(pipe.transformer))
    stats = []
    for i, wrapper in enumerate(wrappers):
        last = wrapper.last_stats
        item = {
            "wrapper_index": i,
            "layer_idx": wrapper.layer_idx,
            "calls": wrapper.calls,
            "stride": wrapper.stride,
            "residual_scale": wrapper.residual_scale,
        }
        if last is not None:
            item.update(last.to_dict())
        stats.append(item)
    return {
        "num_wrappers": len(wrappers),
        "wrapped_layers": list(getattr(pipe.transformer, "sparse_dense_layers", [])),
        "last_stats": stats,
    }
...
def _make_side_by_side(
    ...
    for i in range(count):
        ...
        short_prompt = prompts[i][:70]
        draw.text((4, 4), "baseline", fill="black")
        draw.text((w + 4, 4), "sparse_dense", fill="black")
        draw.text((4, 20), short_prompt, fill="black")
    ...
    grid_path = output_dir / "grid_baseline_vs_sparse_dense.png"
    grid.save(grid_path)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--sparse-layers", default=None, help="Comma-separated explicit layer indices.")
    p.add_argument("--sparse-start-layer", type=int, default=None)
    p.add_argument("--sparse-end-layer", type=int, default=None)
    p.add_argument("--sparse-every", type=int, default=None)
    p.add_argument("--sparse-stride", type=int, default=None)
    p.add_argument("--sparse-residual-scale", type=float, default=None)
    p.add_argument("--sparse-pool-mode", default=None, choices=["avg", "max"])
    p.add_argument("--sparse-upsample-mode", default=None, choices=["nearest", "bilinear", "bicubic", "area"])
    p.add_argument("--sparse-min-tokens", type=int, default=None)
...
def main():
    ...
    output_dir = Path(_resolve(args, cfg, "output_dir", "output_dir", "samples/sana_sprint_sparse_dense_eval"))
    ...
    sparse_cfg = dict(_get_nested(cfg, "sparse_dense", {}) or {})
    sparse_cfg.setdefault("enabled", True)
    if args.sparse_layers is not None:
        sparse_cfg["layers"] = [int(x.strip()) for x in args.sparse_layers.split(",") if x.strip()]
    if args.sparse_start_layer is not None:
        sparse_cfg["start_layer"] = args.sparse_start_layer
    if args.sparse_end_layer is not None:
        sparse_cfg["end_layer"] = args.sparse_end_layer
    if args.sparse_every is not None:
        sparse_cfg["every"] = args.sparse_every
    if args.sparse_stride is not None:
        sparse_cfg["stride"] = args.sparse_stride
    if args.sparse_residual_scale is not None:
        sparse_cfg["residual_scale"] = args.sparse_residual_scale
    if args.sparse_pool_mode is not None:
        sparse_cfg["pool_mode"] = args.sparse_pool_mode
    if args.sparse_upsample_mode is not None:
        sparse_cfg["upsample_mode"] = args.sparse_upsample_mode
    if args.sparse_min_tokens is not None:
        sparse_cfg["min_tokens"] = args.sparse_min_tokens
    ...
    wrapped_layers = apply_sparse_dense_token_blocks(pipe.transformer, sparse_cfg)
    print(f"sparse-dense wrapped layers: {wrapped_layers}")
    sparse_dense = _run_generation(pipe, method_name="sparse_dense", **common_kwargs)
    runtime_summary = _sparse_dense_runtime_summary(pipe)

    side_by_side = None
    if output_type == "pil":
        side_by_side = _make_side_by_side(
            baseline_dir=output_dir / "baseline" / "images",
            sparse_dir=output_dir / "sparse_dense" / "images",
            prompts=prompts,
    ...
    sparse_total = sparse_dense["summary"]["total_wall_ms"]["mean"]
    base_transformer = baseline["summary"]["components"].get("transformer_forward", {}).get("mean", 0.0)
    sparse_transformer = sparse_dense["summary"]["components"].get("transformer_forward", {}).get("mean", 0.0)
    comparison = {
        "sparse_dense": sparse_cfg,
        "sparse_dense_runtime": runtime_summary,
        ...
        "sparse_dense_total_ms": sparse_total,
        "baseline_transformer_ms": base_transformer,
        "sparse_dense_transformer_ms": sparse_transformer,
